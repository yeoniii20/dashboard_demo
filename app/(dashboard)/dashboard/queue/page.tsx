...
import { QUEUES } from "@/app/data/queue";
...
export default function QueueDashboardContent() {
  ...
  const filteredQueues = useMemo(
    () =>
      QUEUES.filter((q) => q.serverId === selectedServerId).filter((q) =>
        ...
        <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
          <div className="space-y-1">
            ...
            <p className="text-12r text-text-soft">
              서버별 큐의 pendingMessages 트렌드를 모니터링합니다.
            </p>
            {selectedServer && (
          ...
          <div className="flex flex-col gap-2 md:flex-row md:items-center">
            ...
            <div className="flex flex-col gap-1 md:ml-3">
              <label className="text-11 text-text-soft">Search queue</label>
              <input
                value={searchTerm}
                onChange={(e) => setSearchTerm(e.target.value)}
                placeholder="queue.payment, notification..."
        ...
        {filteredQueues.length === 0 ? (
          ...
          >
            {filteredQueues.map((queue) => {
              const latest =
                queue.metrics[queue.metrics.length - 1]?.value ?? 0;
              const maxPending = queue.metrics.reduce(
                    ...
                    <div className="flex-1">
                      ...
                      <div className="mt-1 flex flex-wrap items-center gap-1 text-[11px] text-text-deep">
                        ...
                        <span className="hidden xl2:inline">
                          · max:{" "}
                          <span className="text-warning">{maxPending}</span>
                        </span>
                      </div>
                    </div>
